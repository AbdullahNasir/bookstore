...
angular.module('bookStoreApp')
  .controller('BillPurchaseEditCtrl', function ($scope,customerFactory,publishers,books,setFactory,billFactory,$routeParams) {
        ...
        billFactory.getOnePurchaseBill($routeParams.id,function(Data){
            console.log(Data[0]);
            $scope.Bill = Data[0];
        ...
        $scope.saveBill = function () {
            ...
            billFactory.updatePurchaseBill($routeParams.id,function(response){
                console.log(response);
                if(response.success){
                    alert('بل محفوظ ہو گیا ہے.');
                    window.location = "#/bill/purchase/edit/"+$routeParams.id
                }
                else{
        ...
        $scope.deleteBill = function(){
            ...
            billFactory.deletePurchaseBill($routeParams.id,function(response){
                console.log(response);
                if(response.success){
                    alert('بل ڈیلیٹ ہو گیا ہے');
                    window.location = "#/bill/list";
                }
                else{
